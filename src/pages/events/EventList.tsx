...
export default function EventList() {
  ...
  const fetchEvents = useCallback(async (params: Pagination = {
    page: 1,
    take: 10
  }) => {
    try {
  ...
  const handleTableChange = (pagination: TableParams['pagination']) => {
    fetchEvents({ page: pagination?.current ?? 1, take: pagination?.pageSize ?? 10 });
  };
  ...
  const columns = [
    { 
      ...
      dataIndex: 'cover', 
      key: 'cover',
      render: (cover: { url: string }) => cover ? <img src={cover.url} alt="封面圖" style={{ width: 50, height: 50 }} /> : 'N/A',
    },
    ...
    { title: '開始時間', dataIndex: 'startDate', key: 'startDate' },
    {
      ...
      render: (_: unknown, record: EventListItem) => (
        <Button onClick={() => navigate(`/events/${record.id}`)} icon={<EditOutlined />}>
          編輯
        </Button>
      ),
    },
